...
Caso desejarmos obter um polinômio $P$ tal que $P(X_i)=Y_i$, podemos utilizar a técnica chamada >>interpolação de Lagrange.>>Introdução
...
Dessa forma, >>podemos somar todos os polinômios suporte.>>Dedução

$P(X) = Q_1(X_1)+Q_1(X_1)+...+Q_N(X_N)$, já que $P(X_i)=0 + 0 + ... + Q_i(X_i) + 0 + 0 + ... + 0 = Y_i$.
Dessa forma, obtemos um polinômio $P(X)$ de **grau máximo N**.
A >>forma final do polinômio obtido por interpolação de Lagrange será:>>Forma Final
