...
Para se calcular a velocidade do som nesse fluido, pode-se >>proceder da seguinte forma:>>Cálculo da Velocidade Do Som

Considere um momento $t_1$ em que a frente de onda percorreu uma distância $\Delta x = t_1 v$. O volume que em $t_0$ se encontrava antes de $\Delta x$ era de $A t_1 v$ e em $t_1$, dado que as partículas se deslocaram com velocidade $u$, $\Delta V = -A t_1 u$
...
Portanto, >>podemos solucionar o sistema>>Solução Do Sistema
...
Agora, podemos desenvolver esse resultado para um fluido específico: >>os gases>>Som Nos Gases
...
A partir dessa expressão, podemos determinar o módulo de Bulk de um gás >>de outra maneira>>Velocidade Do Som Nos Gases
